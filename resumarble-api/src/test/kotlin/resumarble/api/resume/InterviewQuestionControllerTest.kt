    ...
    init {
        ...
        describe("InterviewQuestionController") {
            ...
            context("면접 예상 질문을 생성 요청하면") {
                coEvery { interviewQuestionFacade.generateInterviewQuestion(any()) } returns responseOneRequest
            ...
            context("면접 예상 질문 생성에 실패하면") {
                coEvery { interviewQuestionFacade.generateInterviewQuestion(any()) } throws CompletionFailedException()
                it("400을 반환한다.") {
                    sut.perform(
                        post("/resumes/interview-questions")
                            .contentType(MediaType.APPLICATION_JSON)
